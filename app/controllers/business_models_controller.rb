class BusinessModelsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @business_model.save
        format.html { redirect_to business_model_url(@business_model), notice: "Business model was successfully created." }
  ...
  def update
    respond_to do |format|
      if @business_model.update(business_model_params)
        format.html { redirect_to business_model_url(@business_model), notice: "Business model was successfully updated." }
  ...
  private
    ...
    def set_business_model
      @business_model = BusinessModel.friendly.find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def business_model_params
      params.require(:business_model).permit(:name, :description, :slug)
    end
end
